# -*- coding: utf-8 -*-
"""Q4 批量测试 runner (与 batch_q3.py 同构).

连续跑 N 轮真实模拟器演练, 每轮:
    /enter -> Policy3(环抱证书 26 点骨架) -> /exit
产物:
    1) 每轮一条 jsonl 原始日志 (默认 cmc26/logs_q4_batch/)
    2) 每轮一条 console 文本日志 (.log)
    3) 逐轮追加的 summary.csv (崩溃也保留已完成轮次)

用法:
    python cmc26/q4/batch_q4.py --rounds 10
    python cmc26/q4/batch_q4.py --rounds 20 --gap 5 --wait 900

注意: 跑之前模拟器必须处于【问题4】模块的演练窗口。
"""
import argparse
import csv
import datetime
...
_ROOT = os.path.dirname(_HERE)                # cmc26/ (含 robot.py)
for _p in (_ROOT, _HERE, os.path.join(_ROOT, "q2")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

import requests                          # noqa: E402
import robot as R                        # noqa: E402
from policy import Policy3               # noqa: E402

DEFAULT_OUTDIR = os.path.join(_ROOT, "logs_q4_batch")
CSV_FIELDS = [
...
def enter_round(sim, wait_s, rid, log):
    ...
    while True:
        ...
        if code == 200 and resp and resp.get("accepted"):
            ...
            log.write(json.dumps({"t": datetime.datetime.now().isoformat(),
                                  "rid": rid, "path": "/enter",
                                  "http": code, "resp": resp},
                                 ensure_ascii=False) + "\n")
            log.flush()
...
def run_one_round(idx, args, outdir, csv_path):
    ...
    try:
        ...
        try:
            ...
            row["enter_ok"] = 1
            print("[ENTER] 成功进入, 开始 Q4 v3 环抱证书策略")
...
def main():
    ...
    ap.add_argument("--wait", type=int, default=900, help="每轮等待窗口开启的秒数")
    ap.add_argument("--gap", type=float, default=5.0, help="两轮之间的现实间隔秒")
    args = ap.parse_args()
    ...
    print(f"robot_id={args.team}  问题4  接口 {args.base}")
